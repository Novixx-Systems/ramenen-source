...
; Modified for Ramenen by Novixx Systems
...
extern os_string_lowercase
extern os_string_compare
extern os_string_chomp
extern os_string_copy
extern os_bcd_to_int

extern os_fatal_error

global os_change_device
global os_get_device
...
;	mov [errno], cx
  ...
  .start_entry:
	...
	test al, 12h			; Is this a hidden file?
	jnz .next_entry			; Yes, ignore it
...
.end:
	...
	.err_msg_floppy_reset	db 'DEVICE_READ_ERROR', 0
...
; --------------------------------------------------------------------------
; os_write_hidden_file -- Save (max 64K) hidden file to disk
; IN: AX = filename, BX = data location, CX = bytes to write
; OUT: Carry clear if OK, set if failure

os_write_hidden_file:
	pusha
...
.carry_on:
	...
	call os_create_hidden_file		; Create empty root dir entry for this file
	jc near .failure		; If we can't write to the media, jump out
...
; --------------------------------------------------------------------------
; os_create_hidden_file -- Creates a new 0-byte (hidden) file on the disk
; IN: AX = location of filename; OUT: Nothing

os_create_hidden_file:
	pusha
...
.found_free_entry:
	...
	mov byte [di+11], 0x02		; Attributes (set archive)
	mov byte [di+12], al		; Reserved
	mov byte [di+13], al		; Reserved
	mov [di+14], cx			; Creation time (now)
	mov [di+16], dx			; Creation date (today)
...
; CHANGE DEVICE ROUTINE
; IN: ah = device letter
; OUT: al = return code (1 = OK, 0 = FAIL)
os_change_device:
	pusha
	mov [bootdev], ah
.chg_drive:
	...
	jge .decrease_first
.continue_then:
	call disk_reset_floppy		; Floppy has been changed
	jc .error			   	    ; Did the floppy reset OK?
	popa
	mov al, 1
	ret
.decrease_first:
	mov al, [bootdev]
	mov byte [bootdev], 80h
	mov byte [temp1], al
	sub byte [temp1], 2
	mov al, [temp1]
	add byte [bootdev], al
